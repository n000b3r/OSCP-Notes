...
* Used this [tool](https://github.com/rewardone/OSCPRepo/blob/master/scripts/recon\_enum/smbver.sh) but changed `tap0` to `tun0`
...
<summary>CrackMapExec (Brute force SMB)</summary>
...
```bash
crackmapexec smb remaining_ips -u usernames.txt -p passwords.txt --continue-on-success
```
